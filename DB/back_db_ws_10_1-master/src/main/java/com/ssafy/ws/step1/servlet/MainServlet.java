...
import com.ssafy.ws.step1.dao.CarDao;
import com.ssafy.ws.step1.dao.CarDaoImpl;
...
public class MainServlet extends HttpServlet {
	...
	// 데이터베이스를 처리해야 할 필요가 생기면 -> DAO 호출
	// MainServlet은 DAO에 의존.
	// 지금은 의존성을 객체를 직접 넣어주고 있다.
	// MainServlet이라는 클래스 안에 다른 객체를 직접 넣어줌.
	private static CarDao dao = CarDaoImpl.getInstance();
	...
	private void doRegistForm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			request.setAttribute("carCount", dao.carCount());
	...
	// regist.jsp => POST요청 => main?action=regist 을 보낸 상황.
	...
	private void doRegist(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			session.setAttribute("carCount", dao.carCount());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void doList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			request.setAttribute("cars", dao.selectAllCars());
			request.setAttribute("carCount", dao.carCount());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
